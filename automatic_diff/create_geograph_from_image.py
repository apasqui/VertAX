...
input_path = "input_image.tiff"
image = tiff.imread(input_path).astype(np.uint16)
image_shape = image.shape
L_box = image_shape[0]

# Segment and refine and pad the image
#input_image = refine_and_pad(segment(image))
input_image = tiff.imread('refined_and_padded_image.tiff').astype(np.uint16)
...
# Loading vertices, edges, faces
vertices = np.load('./vertices.npy')
edges = []
with open('edges.txt', 'r') as file:
    for line in file:
        # Split by tab, and convert each element back to an integer
...
if show:
    ...
    # label = input_image
    # img = image
    # DW = geometry_reconstruction_2d(label,expansion_labels=0, min_dist=3,original_image=img)
    # DW.extended_plot()
    # plt.show()
